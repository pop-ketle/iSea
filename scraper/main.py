...
def archive_db(DATABASE_PATH=DATABASE_PATH, MAX_CNT=3):
    '''もしもの時のために、data.dbを、最大MAX_CNT個、ARCHIVE_FOLDERの中に保存する
    '''
    ARCHIVE_FOLDER = DATABASE_PATH+'db_archives/'
...
def make_urls_dict(MONTHLY_URLS_DICT='./shikyo_monthly_urls_dict.pkl', DAILY_URLS_DICT='./shikyo_daily_urls_dict'):
    '''いわて大漁ナビの市況日報データへ各日毎のページへのurlを辞書形式でまとめる
        monthly_urls_dict: f'{year}-{month}'をキー、各月へのurlを値としたdictを作成して.pkl形式で保存しておく
        daily_urls_dict:   f'{place}-{year}-{month}-{day}'をキー、urlを値としたdictを作成して.pkl形式で保存しておく
        monthly_urls_dictとdaily_urls_dictの差分を見て最低限のクロウルだけで済ませる
    '''
    ...
    monthly_urls_dict     = dict(new_monthly_urls_dict.items() - monthly_urls_dict.items())

    for url in tqdm(monthly_urls_dict.values(), total=len(monthly_urls_dict)):
        time.sleep(1) # 1sウェイト
...
if __name__ == "__main__":
    # もしもの時のために、data.dbを、最大MAX_CNT個、ARCHIVE_FOLDERの中に保存する
    archive_db(DATABASE_PATH=DATABASE_PATH, MAX_CNT=3)
